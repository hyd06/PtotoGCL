...
def parse_args():
    ...
    parser.add_argument('-c', dest="classes", default=5, type=int)
    parser.add_argument('-d', dest="dims", default=16, type=int)
    ...
    parser.add_argument('-s', dest="seed", default=42, type=int)
    parser.add_argument('-r', dest="resdir", default="./polars/", type=str)
...
if __name__ == "__main__":
    ...
    for i in range(args.epochs):
        ...
        prototypes = nn.Parameter(F.normalize(prototypes, p=2, dim=1))
        optimizer = optim.SGD([prototypes], lr=args.learning_rate, momentum=args.momentum)
        print("{:03d}/{:d}: {:.4f}\r".format(i, args.epochs, sep), end='')
        sys.stdout.flush()
    print

    # Store result.
    np.save(args.resdir + "polars-%d-%d.npy" % (args.classes, args.dims), prototypes.data.numpy())
